...
import { ChevronLeft, Check, Camera, User, MapPin, Briefcase, ShieldCheck } from 'lucide-react'
...
export default function ProviderOnboardingPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: profile }, { data: pp }] = await Promise.all([
        ...
        supabase.from('provider_profiles').select('service_categories, main_category').eq('id', user.id).single(),
      ])
  ...
  const canProceedStep1 = () => fullName.trim().length >= 2 && isValidPhone(phone)
  ...
  const goNext = async () => {
    ...
    if (step === 1 && canProceedStep1()) {
      ...
      await supabase.from('profiles').update({ city: city.trim() }).eq('id', user.id)
      setDirection(1)
      setStep(3)
    }
  }
  ...
  return (
    <div className="min-h-dvh bg-[#F4F7FA] flex flex-col overflow-hidden">
      {/* Top bar */}
      <div className="w-full px-4 sm:px-6 py-4 flex items-center justify-between bg-white border-b border-slate-200/60">
        ...
        <span className="text-sm text-slate-500 font-medium">Adım {step} / 4</span>
      </div>
      ...
      {/* Step dots */}
      <div className="flex items-center justify-center gap-6 sm:gap-8 py-4 bg-white">
        {STEP_LABELS.map((s, i) => {
          const stepNum = i + 1
        ...
        <AnimatePresence mode="wait" custom={direction}>
          {step === 1 && (
            ...
            >
              ...
              <p className="text-slate-500 text-center mb-6 max-w-sm text-sm">Adını ve telefon numaranı girerek hemen başla.</p>
          ...
          {step === 2 && (
            ...
            >
              <div className="relative w-72 h-44 mb-4">
                <Image src="/images/onboard-city.png" alt="Şehir" fill className="object-contain" />
              </div>
              <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 text-center mb-1">Neredesin?</h2>
              <p className="text-slate-500 text-center mb-6 max-w-sm text-sm">Şehrini seç, sana en yakın hizmetleri gösterelim.</p>
          ...
          {step === 3 && (
            ...
            >
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 text-center mb-1">Hangi alanda hizmet veriyorsun?</h2>
              <p className="text-slate-500 text-center mb-6 text-sm">Ana kategorini ve sunduğun hizmetleri seç.</p>
      ...
      {/* Bottom nav: Geri / Devam Et */}
      {step <= 2 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 flex items-center justify-between gap-4 safe-area-pb">
          {step > 1 ? (
            <button
              type="button"
              onClick={goBack}
