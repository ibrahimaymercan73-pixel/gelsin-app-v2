...
export default function SoforTekliflerimPage() {
    ...
    const load = async () => {
      ...
      if (error) {
        ...
        const rows: OfferRow[] = (data || []).map((o: any) => ({
          ...
          job: {
            id: String(o.jobs?.id),
            title: o.jobs?.title || 'Şoför Talebi',
            pickup:
  ...
  const statusClass = (s: string | null) => {
    ...
    return 'bg-indigo-900/80 text-slate-200 border-indigo-600'
  }
        ...
        {offers.length === 0 ? (
          ...
          <ul className="space-y-3 text-xs">
            {offers.map((o) => (
              <li
                key={o.id}
                className="rounded-2xl border border-indigo-700 bg-indigo-900/60 p-4 space-y-3"
              >
                ...
                {String(o.status || '').toLowerCase() ===
                  'accepted' && (
                  <div className="pt-2 border-t border-indigo-700 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 px-3 py-2 rounded-xl bg-indigo-950 text-slate-100 border border-indigo-600 text-[11px]"
                      >
                        Navigasyonu Aç
                      </button>
                      <button
