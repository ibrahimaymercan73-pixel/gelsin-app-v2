...
type CekiciMeta = {
  ...
  iban?: string
  documents?: { license?: string; registration?: string }
}
...
function parseMeta(bio: string | null): CekiciMeta | null {
  if (!bio) return null
  try {
    const obj = JSON.parse(bio)
    return obj && typeof obj === 'object' ? (obj as CekiciMeta) : null
  } catch {
    return null
  }
}
...
export default function AdminCekiciPage() {
  ...
  const list = useMemo(() => {
    return rows
      ...
      .filter(({ meta }) => (meta?.type || '').toLowerCase() === 'cekici_onboarding')
  }, [rows])
  ...
  const setStatus = async (providerId: string, next: 'approved' | 'rejected') => {
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        user_id: providerId,
        title: next === 'approved' ? '✅ Çekici başvurun onaylandı' : '❌ Çekici başvurun reddedildi',
        body: next === 'approved'
          ? 'Artık çekici ilanlarını görebilir ve teklif verebilirsin.'
          : 'Başvurun reddedildi. Lütfen bilgilerini kontrol edip tekrar dene.',
        type: next === 'approved' ? 'provider_approved_cekici' : 'provider_rejected_cekici',
      } as any)
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-slate-900">🚛 Çekici Başvuruları</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {list.map(({ r, meta }) => {
              ...
              const vehicleType = meta?.vehicle?.vehicleType || '—'
              const plate = meta?.vehicle?.plate || '—'
              const cities = Array.isArray(meta?.serviceCities) ? meta!.serviceCities! : []
              const docs = {
                ruhsat: meta?.documents?.registration || r.id_document_url,
                ehliyet: meta?.documents?.license || r.criminal_record_url,
              }
              return (
                <div key={r.id} className="rounded-2xl bg-white border border-slate-200 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-slate-700">
                        <span className="font-semibold">Araç:</span> {vehicleType} · <span className="font-semibold">Plaka:</span> {plate}
                      </p>
                      {cities.length > 0 && (
                        <p className="text-xs text-slate-500">
                          <span className="font-semibold">Şehirler:</span> {cities.slice(0, 8).join(', ')}{cities.length > 8 ? '…' : ''}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <DocThumb label="Ruhsat" url={docs.ruhsat} onOpen={setLightbox} />
