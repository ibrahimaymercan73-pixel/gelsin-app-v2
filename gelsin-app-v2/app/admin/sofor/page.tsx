      ...
      if (error) {
        ...
        setItems(
          (data || []).map((p: any) => ({
            ...
            phone: p.phone,
            licence_class: p.licence_class,
            vehicle_preferences: Array.isArray(p.vehicle_preferences)
              ? p.vehicle_preferences
              : [],
  ...
  const updateStatus = async (row: ProviderRow, status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        title: status === 'approved' ? 'Şoför başvurunuz onaylandı' : 'Şoför başvurunuz reddedildi',
        body:
          status === 'approved'
            ? 'Artık şoför taleplerini görebilir ve teklif verebilirsiniz.'
                ...
                >
                  ...
                  <div className="space-y-1">
                    <p className="text-slate-700">
                      Ehliyet: {p.licence_class || '—'}
                    </p>
                    <p className="text-slate-500 line-clamp-2">
                      Araçlar:{' '}
                      {p.vehicle_preferences.length
                        ? p.vehicle_preferences.join(', ')
                        : '—'}
                    </p>
                  </div>
